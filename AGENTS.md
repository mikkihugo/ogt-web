...
- Secrets are managed exclusively via git-crypt and `.env.encrypted`. All secrets are encrypted at rest and unlocked automatically in the Nix shell. Use the provided scripts for key management and secrets sync.
- Tasks: use the `Justfile` helpers when outside the shell; they bootstrap the dev env automatically. Media helpers: `just sync-media` (push `pub/media` to S3) and `just restore-media` (pull from S3).
