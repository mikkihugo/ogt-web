...
export const scoreSuppliersJob = async (container: MedusaContainer) => {
    ...
    try {
        ...
        for (const stat of stats) {
            ...
            logger.info(`Supplier ${stat.supplier_id}: ${failures}/${total} failures. Score: ${score}`);
            ...
            await pool.query(`
                ...
                VALUES ($1, $2, $3)
            `, ['supplier_score_update', stat.supplier_id, JSON.stringify({
                old_score: 'unknown',
                new_score: score,
        ...
        logger.error("Failed to score suppliers", err);
    }
};
