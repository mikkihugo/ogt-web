...
import { InventoryService, SupplierOffer } from "../modules/dropship/inventory.service";
...
export const syncSupplierInventory = async (
...
GG-002,0,20.00`;
        ...
        for (const [variantId, qty] of Object.entries(updates)) {
            try {
                ...
                logger.error(`Failed to update variant ${variantId}`, err);
            }
        }
        ...
        logger.error(`Sync failed for supplier ${supplierId}`, err);
        throw err;
    }
};
