import { Container, Heading, Table, Button, Input, Textarea, Toaster, useToast } from "@medusajs/ui";
...
const ShopManagement = () => {
    ...
    useEffect(() => {
        fetch("/admin/ops/shops")
            ...
            .catch(err => toast({ title: "Error", description: "Failed to load shops", variant: "error" }));
    }, []);
            ...
            <Container>
                ...
                <div className="flex flex-col gap-4 mt-4">
                    ...
                    <Textarea
                        ...
                        onChange={(e) => {
                            try { setEditing({ ...editing, marketing_config: JSON.parse(e.target.value) }) } catch { }
                        }}
                    />
                    ...
                    <Textarea
                        ...
                        onChange={(e) => {
                            try { setEditing({ ...editing, theme_config: JSON.parse(e.target.value) }) } catch { }
                        }}
                    />
    ...
    return (
        <Container>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={() => setEditing({ id: 'new_shop', name: 'New Shop', domains: [], marketing_config: {}, theme_config: {} })}>
                    Create Shop
                </Button>
            </div>
                ...
                <Table.Body>
                    {shops.map(shop => (
                        <Table.Row key={shop.id}>
                            ...
                            <Table.Cell>
                                <Button variant="secondary" onClick={() => setEditing(shop)}>Edit Config</Button>
                            </Table.Cell>
                        </Table.Row>
                    ))}
                </Table.Body>
