...
export default async function (container: MedusaContainer) {
    ...
    await ops.getQueryRunner().then(pool =>
        pool.query(`
    ...
    const { OrderRoutingService } = await import("../modules/dropship/order-routing.service");
    ...
    await routingService.routeOrder(
        "ord_test_001",
        [{ id: "item_test_1", variant_id: "variant_123", quantity: 5, unit_price: 1500 }],
        "shop_us"
    );

    // Check PO existence
    const opsRouting = (supplierService as any).ops;
    const { rows: poRows } = await opsRouting.getQueryRunner().then(pool =>
        pool.query(`SELECT * FROM dropship.purchase_order WHERE medusa_order_id = 'ord_test_001'`)
    );

    if (poRows.length > 0) {
        logger.info(`SUCCESS: PO created: ${poRows[0].id} for Supplier ${poRows[0].supplier_id}`);
    } else {
        logger.error("FAILURE: No PO created for routed order.");
        // Check exceptions
        const { rows: excRows } = await opsRouting.getQueryRunner().then(pool =>
            pool.query(`SELECT * FROM dropship.ops_exception WHERE entity_ref->>'orderId' = 'ord_test_001'`)
        );
        if (excRows.length > 0) {
            logger.warn(`Routing Exception Found: ${excRows[0].type} - ${excRows[0].notes}`);
        }
    }
