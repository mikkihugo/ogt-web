...
export default async function seedBlogEnterprise({ container }: ExecArgs) {
    ...
    // Get Channels
    const [usChannel] = await salesChannelService.listSalesChannels({ name: "US Store" })
    const [euChannel] = await salesChannelService.listSalesChannels({ name: "EU Store" })
    ...
    const posts = [
        ...
        },
        {
            ...
            category: "wellness",
            published_at: new Date(),
            channel_config: (usChannel && euChannel) ? { ids: [usChannel.id, euChannel.id] } : null,
            language_code: "en-US"
        }
    ]
