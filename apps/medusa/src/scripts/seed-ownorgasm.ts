...
export default async function seedOwnOrgasm({ container }: ExecArgs) {
  ...
  // Cast to any because the method signatures (createPosts) are dynamic/generated and not in the base interface yet
  const blogModuleService = container.resolve(BLOG_MODULE) as any;

  logger.info("Seeding OwnOrgasm Channel...");

  // 1. Create Sales Channel
  let channel = await salesChannelService
    .listSalesChannels({ name: "OwnOrgasm" })
    .then((c) => c[0]);
  if (!channel) {
