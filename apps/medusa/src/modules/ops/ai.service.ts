...
export class AiOpsService {
    ...
    private async retrieveContext(question: string) {
        ...
        try {
            queryVector = await this.generateEmbedding(question);
        } catch (e) {
        ...
        if (queryVector.length > 0) {
            ...
            const { rows: semanticRows } = await runner.query(vectorQuery, [vectorStr]);
        ...
        if (question.toLowerCase().includes("supplier")) {
            const { rows } = await runner.query(`SELECT name, reliability_score FROM dropship.supplier`);
            context += `Supplier Current Status: ${JSON.stringify(rows)}\n`;
        }


        // 3. Fetch Suppressed Products
        if (question.toLowerCase().includes("product") || question.includes("suppress")) {
        ...
        if (context === "") {
            context = "No specific database records found for this query. Answer based on general knowledge of the system.";
        }
