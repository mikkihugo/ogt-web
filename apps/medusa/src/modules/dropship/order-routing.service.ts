...
export class OrderRoutingService {
     ...
     */
    async routeOrder(orderId: string, items: { id: string, variant_id: string, quantity: number, unit_price: number }[], shopId: string) {
        this.logger?.info(`Routing order ${orderId} for shop ${shopId} with ${items.length} lines.`);
        ...
        for (const item of items) {
            try {
                ...
                const supplier = await this.findBestSupplier(item.variant_id, item.quantity);
                ...
                if (!poGroups[supplier.supplier_id]) {
                    poGroups[supplier.supplier_id] = { supplierId: supplier.supplier_id, lines: [] };
                }
                ...
                await this.exceptionService.raise(
                    ...
                    "ROUTING_ERROR",
                    "critical",
                    { orderId, lineItemId: item.id, error: err.message }
                );
            }
        }
        ...
        for (const group of Object.values(poGroups)) {
            await this.createPurchaseOrder(orderId, shopId, group.supplierId, group.lines);
        }
    }

    private async findBestSupplier(variantId: string, qtyNeeded: number): Promise<RoutingCandidate | null> {
    ...
    `;
        ...
        const { rows } = await this.ops.getQueryRunner().then(pool =>
            pool.query(q, [variantId, qtyNeeded])
        );
    ...
    private async createPurchaseOrder(medusaOrderId: string, shopId: string, supplierId: string, lines: any[]) {
        ...
        for (const line of lines) {
            ...
            await client.query(
               ...
               VALUES ($1, $2, $3, $4, $5, $6)`,
                [lineId, poId, line.medusa_item_id, line.supplier_sku, line.qty, line.cost]
            );
        }
        ...
        try {
            ...
            this.logger?.warn("Failed to log routing decision history", e);
        }
    }
}
