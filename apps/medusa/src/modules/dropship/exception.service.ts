...
export class ExceptionService {
    ...
    `;
        ...
        await this.ops.getQueryRunner().then(pool =>
            pool.query(q, [id, shopId, type, severity, JSON.stringify(entityRef), notes])
        );
    }
    ...
    async listOpen(shopId?: string): Promise<OpsException[]> {
        ...
        const { rows } = await this.ops.getQueryRunner().then(pool => pool.query(q, params));
        return rows;
    }
}
