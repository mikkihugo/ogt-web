...
export class InventoryService {
    ...
    async upsertSupplierOffers(supplierId: string, offers: SupplierOffer[]) {
        ...
        for (let i = 0; i < offers.length; i += batchSize) {
            ...
            DO UPDATE SET 
              ...
              fetched_at=now()`,
                    [supplierId, offer.sku, offer.qty, offer.cost, offer.currency, offer.leadTimeDays, offer.shipsFrom]
                );
            }
        }
    }
     ...
     */
    async calculateMedusaUpdates(supplierId: string): Promise<Record<string, number>> {
        const client = await this.ops.getQueryRunner();
